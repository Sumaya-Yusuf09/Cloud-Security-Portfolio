...
![Groups created in Azure](Groups_Created-Azure.png)
...
![Adding grant control to the new policy](Adding_control_access_to_the_new_policy.png)

![New policy excluding my own account](New_policy_for_all_users_except_Sumaya_Yusuf.png)

![Policy showing as on in the list](MFA_policy_is_on.png)
...
![Both policies now showing in the list](Policy_List.png)
...
![Testing the sign in with What If](Testing_-_What_if__policy_.png)

![Evaluation result showing which policies apply](Testing__1__-_What_if__Policy_.png)
...
![Setting activation duration and approval requirement](Setting_Assignment_duration___Requiring_approval_upon_acitivations.png)

![Adding the eligible assignment for IT Admins](Assigned_Security_Reader_to_IT_Admins.png)

![Assignment now showing as eligible](Assignment_List.png)

Here is what it looks like when someone actually needs that access. They submit a request with a reason.

![Requesting activation with a written reason](Assignment_Activation_Request.png)

And then someone else has to approve it before it becomes active.

![Approving the activation request](Approve_Assignment_Request.png)
...
![Finance users group before offboarding](Finance_User_List.png)

When someone leaves, the first thing to do is stop them from being able to sign in at all, before anything else.

![Disabling the account](Blocking_Moa_Ali_from_signing_in_-_Offboarding.png)

![Account now shows as disabled](Account_Disabled_-_Moa_Ali.png)

Then she gets removed from the group that gave her access to finance systems.

![Finance users group after she is removed](Moa_Ali_removed_from_Finance_Users_Group.png)
